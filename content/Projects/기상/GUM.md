...
- **오차 전파 법칙(Error Propagation Law)**:
    ...
    - **예시**: Z=X+YZ = X + Y 일 때, σZ=σX2+σY2\sigma_Z = \sqrt{\sigma_X^2 + \sigma_Y^2}

#### **1.3 측정 결과의 표현**

- **일반적 표현**: 측정값±오차\text{측정값} \pm \text{오차}
    - **예시**: 50.0±0.550.0 \pm 0.5 mm
...
- **합성 표준 불확도(Combined Standard Uncertainty, ucu_c)**:
    
    - A형과 B형 불확도를 모두 고려하여 전체 불확도를 계산.
    - **계산식**: uc=uA2+uB2u_c = \sqrt{u_A^2 + u_B^2}
- **확장 불확도(Expanded Uncertainty, UU)**:
    
    - 합성 표준 불확도에 신뢰 계수(k)를 곱하여 특정 신뢰 수준을 갖는 불확도를 계산.
    - **계산식**: U=k×ucU = k \times u_c
    - **신뢰 수준**: 일반적으로 k=2 (약 95% 신뢰 수준), k=3 (약 99.7% 신뢰 수준)

#### **2.4 측정 결과의 표현**

- **표준화된 표현**: 측정값±U\text{측정값} \pm U (k=값, 신뢰 수준)
    - **예시**: (20.00±0.05)(20.00 \pm 0.05) mL (k=2)
- **의미**: 측정값이 ±U\pm U 범위 내에 실제 값이 존재할 확률이 약 95%임을 의미.
...
- **오차 분석**:
    ...
    - 측정 결과를 측정값±오차\text{측정값} \pm \text{오차} 형태로 단순하게 표현합니다.
...
- **A형 불확도 평가**:
    ...
    - 표준 편차를 이용하여 A형 불확도 uAu_A를 구함.
- **B형 불확도 평가**:
    
    - 저울의 제조사에서 제공하는 최대 허용 오차나 분해능 등의 정보를 활용.
    - 균일 분포나 삼각 분포를 가정하여 B형 불확도 uBu_B를 계산.
- **합성 표준 불확도 및 확장 불확도 계산**:
    
    - uc=uA2+uB2u_c = \sqrt{u_A^2 + u_B^2}
    - U=k×ucU = k \times u_c (일반적으로 k=2 사용)
- **최종 결과의 표현**:
    
    - 측정값±U\text{측정값} \pm U (k=2, 약 95% 신뢰 수준)
    - **예시**: (150.00±0.10)(150.00 \pm 0.10) g (k=2)
...
위의 내용을 통해 오차 분석 체계와 GUM의 불확도 개념의 차이점과 각각의 중요성을 이해하실 수 있기를 바랍니다.
